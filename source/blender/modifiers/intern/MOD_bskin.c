...
static void bmesh_mesh_face_update_normal(BMesh* bm)
{
...
static void isolate_path_verts(SkinData *data)
{
	...
	bmesh_mesh_face_update_normal(bm);
...
static void bridge_vert_set(SkinData	*data, BMesh	*bm, BMVert	**vset0, BMVert	**vset1, int	*order, int valence)
{
	...
	if (split_edge = BM_edge_exists(vset0[0], vset1[order[0]])){
...
static void split_limb_edges(SkinData	*data)
{
	...
	BMIter iter;
	BMesh *bm;
	int eflag, cd_edge_flag_offset, cd_vert_vert_offset, i;
	bm = data->bm;
